...
def test_gain_fiat_crypto_crypto_fiat_trades():
    ...
    # ETH price history is not needed.

    # Trade 1 BTC to 10 ETH
    tx = wallet_helper.trade(btc, 1, eth, 10)
    assert tx.from_detail.cost_basis == Decimal(600)  # 1 BTC == 600 EUR
    # The FIAT value of the BTC at the time of the trade time will be the new cost basis of ETH
    ...
    assert tx.to_detail.cost_basis == Decimal(120)  # 1 ETH == 120 EUR (1200 EUR * 2 BTC / 20 ETH)
